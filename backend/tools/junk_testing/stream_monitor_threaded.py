...
class StreamMonitor:
    def __init__(self, url="https://icecast.teveo.cu/b3jbfThq"):
        self.__thread_data = {}
        if url:
            self.__thread_data[url] = queue.Queue()
    ...
    def get_thread_data_item(self, url):
        try:
            return self.__thread_data[url].get()
        except queue.Empty:
            return None

    def test_thread(self, url=''):
        if url:
            self.__thread_data[url] = queue.Queue()
            self.__ffmpeg_cmd_filters.get('peak_level')[3] = url

        for i in self.ffmpeg_peak_level():
            self.__thread_data[url].put(i)


def main(stdscr, args):
    monitor = StreamMonitor()
    t1 = threading.Thread(target=monitor.test_thread, args=["https://icecast.teveo.cu/7NgVjcqX"])
    t2 = threading.Thread(target=monitor.test_thread, args=["https://icecast.teveo.cu/b3jbfThq"])
    t1.start()
    t2.start()

    while True:
        print("https://icecast.teveo.cu/7NgVjcqX", monitor.get_thread_data_item("https://icecast.teveo.cu/7NgVjcqX"))
        print("https://icecast.teveo.cu/b3jbfThq", monitor.get_thread_data_item("https://icecast.teveo.cu/b3jbfThq"))
